...
function Gondor() {
    ...
    const targetAlphas = [1.7, 1, 1.5, 1.7];
    const targetRadiuses = [160, 100, 80, 150];
    ...
    const minAlpha = 0.4; // Example: -90 degrees
    const maxAlpha = 2.5; // Example: +90 degrees
    const minBeta = 0.2; // Prevent looking straight up
    ...
    useEffect(() => {
        ...
        camera.setPosition(new Vector3(20, 80, 180));
        ...
        skyboxMaterial.reflectionTexture = new CubeTexture.CreateFromPrefilteredData('./textures/syferfontein_6d_clear_puresky_16k.env', scene);
        ...
        SceneLoader.LoadAssetContainer(
            process.env.PUBLIC_URL + '/gondor_compressed.glb', // Path to your GLB file
            ...
            (container) => {
                ...
                container.rootNodes.forEach(rootNode => {
                    //rootNode.position.y -= 15;
                    //rootNode.position.x += 55; // Adjust this value
                    rootNode.position.z -= 50; 
                });
...
// console.log("Material Properties:", material);
  if (material.name === 'Ground 1') { // Check if the material name matches
    // Convert the hex color to a Babylon.js Color3
    const hexColor = "#4a4842";
    ...
    //console.log("Albedo color updated for Material.001");
    }   
}
// else if (material.name === 'Mountain 1') { // Check if the material name matches
//     // Convert the hex color to a Babylon.js Color3
//     const hexColor = "#3d382d";
//     const color = Color3.FromHexString(hexColor);
...
// }
            ...
            (event) => {
                ...
                if (event.total === 0) {
                    ...
                    const estimatedTotal = 7 * 1024 * 1024; // 5MB example
...
return (
   ...
   <div className="loader-container">
      ...
      <div className="loader">
         ...
         <p class="load">{loadingProgress}%</p>
      </div>
   </div>
   )}
   ...
   {textBox1Visible && (
    <div className="menu">
        ...
        <ul className="nav">
          <li>
            <button onClick={() => handleItemClick('problemSolver')}>
              Problem solver
            </button>
          </li>
          <li>
            <button onClick={() => handleItemClick('communicationStyle')}>
              Communication
            </button>
          </li>
          <li>
            <button onClick={() => handleItemClick('others')}>Others
            </button>
          </li>
        </ul>
      </div>
   ...
   {textBox1Visible && selectedItem && (
        <div className="content-panel"
        onWheel={handleDivScrollStart}  
        onMouseLeave={handleDivScrollEnd}> 
           <div className="close-button" onClick={exit}>X</div>

          {selectedItem === 'problemSolver' && (<TextBox choice={1}></TextBox>
          )}
          {selectedItem === 'communicationStyle' && (<TextBox choice={2}></TextBox>
          )}
          {selectedItem === 'others' && (<TextBox choice={3}></TextBox>
          )}
        </div>
      )}
      {textBox2Visible && (
   <div className="text-box3 top-left">
      <div class="info-header">Resume:</div>     
       <a class="thea" href="../Software_Engineer_Hong_Vo_Resume.pdf" download><span>Download</span><span>PDF</span></a>

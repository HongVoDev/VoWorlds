...
function Mordor() {
    ...
    const targetAlphas = [1.6, 1.4, 1.5, 1.5];
    const targetRadiuses = [500, 60, 100, 500];
    const targetXoffset = [0, 0, 30 , -20];
    const animationStates = 3;
    const scrollThresholds = [0, 200, 500, 1100]; // Example thresholds - adjust as needed
...
setOverlayVisible(false)
    ...
    useEffect(() => {
        ...
        camera.setPosition(new Vector3(0,500,500));
        //camera.fov = 0.8;
...
directionalLight.intensity = 3.5; // Stronger for sunset effect
...
directionalLight2.intensity = 0.5; // Stronger for sunset effect
directionalLight2.diffuse = new Color3.FromHexString("#d9b0b0"); // Deep, intense orange color (R=1, G=0.4, B=0)
...
var skybox = MeshBuilder.CreateBox('skyBox', { size: 1200.0 }, scene);
...
scene.environmentTexture = skyboxMaterial.reflectionTexture;
        ...
        SceneLoader.LoadAssetContainer(
            process.env.PUBLIC_URL + '/mordor_compressed.glb', // Path to your GLB file
            ...
            (container) => {
                ...
                container.rootNodes.forEach(rootNode => {
                    rootNode.position.y -= 400;
                    rootNode.position.z -= 100;
      ...
      material._emissiveColor = color;
            ...
            (event) => {
                ...
                if (event.total === 0) {
                    ...
                    const estimatedTotal = 9 * 1024 * 1024; // 5MB example
...
return (
   ...
   {textBox1Visible && <div className="text-box top-left">
      <div class="info-header">Ending:</div>
      <div class="box-text">We have reached Mordor.
      </div>
  </div>}

  {textBox2Visible && (
   <div className="text-box bottom-right2">
      <div class="info-header">Thank you</div>
      <div class="box-text">Even though our journey ended,
        we can still stay in touch!
      </div>
      <div class="contact-buttons">
    <a href="https://www.linkedin.com/in/hongvodev" class="linkedin-button" target="_blank">
      Connect with me on LinkedIn
    </a>
    <a href="mailto:hongvo.dev@gmail.com" class="email-button">
      Email me
    </a>
  </div>
   </div>
